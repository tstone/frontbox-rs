...
impl SystemCommandsProcessor {
  ...
  pub fn process(command: SystemMessage, systems: &mut Vec<SystemContainer>, ctx: &mut Context) {
    Self::process_app(
      command,
      systems,
      ctx.store,
      ctx.app_sender.clone(),
      ctx.system_sender.clone(),
    );
  }

  pub(crate) fn process_app(
