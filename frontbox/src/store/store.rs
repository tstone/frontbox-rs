...
impl Store {
  ...
  pub fn expect<T: StorableType>(&self) -> &T {
    if let Some(value) = self.get::<T>() {
      ...
      self.trace_contents();
      panic!("Expected {} ({:?}) value not found in Store", std::any::type_name::<T>(), TypeId::of::<T>());
    }
  }

  pub fn get_or_default<T: StorableType + Default>(&self) -> &T {
    if !self.internal.contains_key(&TypeId::of::<T>()) {
      log::trace!("{} not found in Store, returning default value", std::any::type_name::<T>());
      T::default();
    }
    self.get::<T>().unwrap()
  }
  ...
  pub fn get_mut<T: StorableType>(&mut self) -> Option<&mut T> {
    log::trace!("Getting mutable reference to {} from Store", std::any::type_name::<T>());
    self
      .internal
      .get_mut(&TypeId::of::<T>())
