// Auto-generated from font.json. Do not edit by hand.
use std::collections::HashMap;
use std::sync::LazyLock;

use crate::pixel_font::{PixelFontCharacterMap, PixelFontGlyph};

/// Sigi Condensed Regular 5px pixel font by Rasmus Andersson
/// CC0 Public Domain License
/// <https://github.com/rsms/sigi-pixel-font>
pub static SIGI_CONDENSED_REGULAR_5PX_FONT: LazyLock<PixelFontCharacterMap> = LazyLock::new(|| {
  let mut glyphs = HashMap::new();

  glyphs.insert(
    '0',
    PixelFontGlyph {
      name: "0",
      width: 3,
      pixels: vec![
        false, true, false, true, false, true, true, false, true, true, false, true, false, true,
        false,
      ],
    },
  );

  glyphs.insert(
    '1',
    PixelFontGlyph {
      name: "1",
      width: 3,
      pixels: vec![
        false, true, false, true, true, false, false, true, false, false, true, false, true, true,
        true,
      ],
    },
  );

  glyphs.insert(
    '2',
    PixelFontGlyph {
      name: "2",
      width: 4,
      pixels: vec![
        false, true, true, false, true, false, false, true, false, false, true, false, false, true,
        false, false, true, true, true, true,
      ],
    },
  );

  glyphs.insert(
    '3',
    PixelFontGlyph {
      name: "3",
      width: 4,
      pixels: vec![
        true, true, true, true, false, false, true, false, false, true, true, false, false, false,
        false, true, true, true, true, false,
      ],
    },
  );

  glyphs.insert(
    '4',
    PixelFontGlyph {
      name: "4",
      width: 4,
      pixels: vec![
        false, false, true, false, false, true, true, false, true, true, true, true, false, false,
        true, false, false, false, true, false,
      ],
    },
  );

  glyphs.insert(
    '5',
    PixelFontGlyph {
      name: "5",
      width: 3,
      pixels: vec![
        true, true, true, true, false, false, true, true, true, false, false, true, true, true,
        false,
      ],
    },
  );

  glyphs.insert(
    '6',
    PixelFontGlyph {
      name: "6",
      width: 4,
      pixels: vec![
        false, true, true, false, true, false, false, false, true, true, true, false, true, false,
        false, true, false, true, true, false,
      ],
    },
  );

  glyphs.insert(
    '7',
    PixelFontGlyph {
      name: "7",
      width: 3,
      pixels: vec![
        true, true, true, false, false, true, false, false, true, false, true, false, false, true,
        false,
      ],
    },
  );

  glyphs.insert(
    '8',
    PixelFontGlyph {
      name: "8",
      width: 4,
      pixels: vec![
        false, true, true, false, true, false, false, true, false, true, true, false, true, false,
        false, true, false, true, true, false,
      ],
    },
  );

  glyphs.insert(
    '9',
    PixelFontGlyph {
      name: "9",
      width: 4,
      pixels: vec![
        false, true, true, false, true, false, false, true, false, true, true, true, false, false,
        false, true, false, true, true, false,
      ],
    },
  );

  glyphs.insert(
    'A',
    PixelFontGlyph {
      name: "A",
      width: 4,
      pixels: vec![
        false, true, true, false, true, false, false, true, true, true, true, true, true, false,
        false, true, true, false, false, true,
      ],
    },
  );

  glyphs.insert(
    'B',
    PixelFontGlyph {
      name: "B",
      width: 4,
      pixels: vec![
        true, true, true, false, true, false, false, true, true, true, true, false, true, false,
        false, true, true, true, true, false,
      ],
    },
  );

  glyphs.insert(
    'C',
    PixelFontGlyph {
      name: "C",
      width: 4,
      pixels: vec![
        false, true, true, false, true, false, false, true, true, false, false, false, true, false,
        false, true, false, true, true, false,
      ],
    },
  );

  glyphs.insert(
    'D',
    PixelFontGlyph {
      name: "D",
      width: 4,
      pixels: vec![
        true, true, true, false, true, false, false, true, true, false, false, true, true, false,
        false, true, true, true, true, false,
      ],
    },
  );

  glyphs.insert(
    'E',
    PixelFontGlyph {
      name: "E",
      width: 3,
      pixels: vec![
        true, true, true, true, false, false, true, true, false, true, false, false, true, true,
        true,
      ],
    },
  );

  glyphs.insert(
    'F',
    PixelFontGlyph {
      name: "F",
      width: 3,
      pixels: vec![
        true, true, true, true, false, false, true, true, false, true, false, false, true, false,
        false,
      ],
    },
  );

  glyphs.insert(
    'G',
    PixelFontGlyph {
      name: "G",
      width: 4,
      pixels: vec![
        false, true, true, true, true, false, false, false, true, false, true, true, true, false,
        false, true, false, true, true, true,
      ],
    },
  );

  glyphs.insert(
    'H',
    PixelFontGlyph {
      name: "H",
      width: 4,
      pixels: vec![
        true, false, false, true, true, false, false, true, true, true, true, true, true, false,
        false, true, true, false, false, true,
      ],
    },
  );

  glyphs.insert(
    'I',
    PixelFontGlyph {
      name: "I",
      width: 3,
      pixels: vec![
        true, true, true, false, true, false, false, true, false, false, true, false, true, true,
        true,
      ],
    },
  );

  glyphs.insert(
    'J',
    PixelFontGlyph {
      name: "J",
      width: 4,
      pixels: vec![
        false, false, false, true, false, false, false, true, false, false, false, true, true,
        false, false, true, false, true, true, false,
      ],
    },
  );

  glyphs.insert(
    'K',
    PixelFontGlyph {
      name: "K",
      width: 4,
      pixels: vec![
        true, false, false, true, true, false, true, false, true, true, false, false, true, false,
        true, false, true, false, false, true,
      ],
    },
  );

  glyphs.insert(
    'L',
    PixelFontGlyph {
      name: "L",
      width: 3,
      pixels: vec![
        true, false, false, true, false, false, true, false, false, true, false, false, true, true,
        true,
      ],
    },
  );

  glyphs.insert(
    'M',
    PixelFontGlyph {
      name: "M",
      width: 5,
      pixels: vec![
        true, false, false, false, true, true, true, false, true, true, true, false, true, false,
        true, true, false, false, false, true, true, false, false, false, true,
      ],
    },
  );

  glyphs.insert(
    'N',
    PixelFontGlyph {
      name: "N",
      width: 4,
      pixels: vec![
        true, false, false, true, true, true, false, true, true, false, true, true, true, false,
        false, true, true, false, false, true,
      ],
    },
  );

  glyphs.insert(
    'O',
    PixelFontGlyph {
      name: "O",
      width: 4,
      pixels: vec![
        false, true, true, false, true, false, false, true, true, false, false, true, true, false,
        false, true, false, true, true, false,
      ],
    },
  );

  glyphs.insert(
    'P',
    PixelFontGlyph {
      name: "P",
      width: 4,
      pixels: vec![
        true, true, true, false, true, false, false, true, true, true, true, false, true, false,
        false, false, true, false, false, false,
      ],
    },
  );

  glyphs.insert(
    'Q',
    PixelFontGlyph {
      name: "Q",
      width: 4,
      pixels: vec![
        false, true, true, false, true, false, false, true, true, false, false, true, true, false,
        true, false, false, true, true, true,
      ],
    },
  );

  glyphs.insert(
    'R',
    PixelFontGlyph {
      name: "R",
      width: 4,
      pixels: vec![
        true, true, true, false, true, false, false, true, true, true, true, false, true, false,
        true, false, true, false, false, true,
      ],
    },
  );

  glyphs.insert(
    'S',
    PixelFontGlyph {
      name: "S",
      width: 4,
      pixels: vec![
        false, true, true, false, true, false, false, false, false, true, true, false, false,
        false, false, true, false, true, true, false,
      ],
    },
  );

  glyphs.insert(
    'T',
    PixelFontGlyph {
      name: "T",
      width: 3,
      pixels: vec![
        true, true, true, false, true, false, false, true, false, false, true, false, false, true,
        false,
      ],
    },
  );

  glyphs.insert(
    'U',
    PixelFontGlyph {
      name: "U",
      width: 4,
      pixels: vec![
        true, false, false, true, true, false, false, true, true, false, false, true, true, false,
        false, true, false, true, true, false,
      ],
    },
  );

  glyphs.insert(
    'V',
    PixelFontGlyph {
      name: "V",
      width: 5,
      pixels: vec![
        true, false, false, false, true, true, false, false, false, true, false, true, false, true,
        false, false, true, false, true, false, false, false, true, false, false,
      ],
    },
  );

  glyphs.insert(
    'W',
    PixelFontGlyph {
      name: "W",
      width: 5,
      pixels: vec![
        true, false, false, false, true, true, false, false, false, true, true, false, true, false,
        true, true, false, true, false, true, false, true, false, true, false,
      ],
    },
  );

  glyphs.insert(
    'X',
    PixelFontGlyph {
      name: "X",
      width: 3,
      pixels: vec![
        true, false, true, true, false, true, false, true, false, true, false, true, true, false,
        true,
      ],
    },
  );

  glyphs.insert(
    'Y',
    PixelFontGlyph {
      name: "Y",
      width: 3,
      pixels: vec![
        true, false, true, true, false, true, false, true, false, false, true, false, false, true,
        false,
      ],
    },
  );

  glyphs.insert(
    'Z',
    PixelFontGlyph {
      name: "Z",
      width: 4,
      pixels: vec![
        true, true, true, true, false, false, true, false, false, true, false, false, true, false,
        false, false, true, true, true, true,
      ],
    },
  );

  glyphs.insert(
    'Å',
    PixelFontGlyph {
      name: "Å",
      width: 4,
      pixels: vec![
        false, true, true, false, false, true, true, false, true, false, false, true, true, true,
        true, true, true, false, false, true,
      ],
    },
  );

  glyphs.insert(
    'Ä',
    PixelFontGlyph {
      name: "Ä",
      width: 4,
      pixels: vec![
        true, false, false, true, false, true, true, false, true, false, false, true, true, true,
        true, true, true, false, false, true,
      ],
    },
  );

  glyphs.insert(
    'Ö',
    PixelFontGlyph {
      name: "Ö",
      width: 4,
      pixels: vec![
        true, false, false, true, false, true, true, false, true, false, false, true, true, false,
        false, true, false, true, true, false,
      ],
    },
  );

  glyphs.insert(
    ' ',
    PixelFontGlyph {
      name: " ",
      width: 3,
      pixels: vec![
        false, false, false, false, false, false, false, false, false, false, false, false, false,
        false, false,
      ],
    },
  );

  glyphs.insert(
    '!',
    PixelFontGlyph {
      name: "!",
      width: 1,
      pixels: vec![true, true, true, false, true],
    },
  );

  glyphs.insert(
    '@',
    PixelFontGlyph {
      name: "@",
      width: 5,
      pixels: vec![
        false, true, true, true, false, true, false, true, false, true, true, false, true, true,
        true, true, false, false, false, false, false, true, true, true, false,
      ],
    },
  );

  glyphs.insert(
    '#',
    PixelFontGlyph {
      name: "#",
      width: 5,
      pixels: vec![
        false, true, false, true, false, true, true, true, true, true, false, true, false, true,
        false, true, true, true, true, true, false, true, false, true, false,
      ],
    },
  );

  glyphs.insert(
    '$',
    PixelFontGlyph {
      name: "$",
      width: 5,
      pixels: vec![
        false, true, true, true, false, true, false, true, false, false, false, true, true, true,
        false, false, false, true, false, true, false, true, true, true, false,
      ],
    },
  );

  glyphs.insert(
    '%',
    PixelFontGlyph {
      name: "%",
      width: 5,
      pixels: vec![
        true, true, false, false, true, true, true, false, true, false, false, false, true, false,
        false, false, true, false, true, true, true, false, false, true, true,
      ],
    },
  );

  glyphs.insert(
    '^',
    PixelFontGlyph {
      name: "^",
      width: 3,
      pixels: vec![
        false, true, false, true, false, true, false, false, false, false, false, false, false,
        false, false,
      ],
    },
  );

  glyphs.insert(
    '&',
    PixelFontGlyph {
      name: "&",
      width: 4,
      pixels: vec![
        false, true, false, false, true, false, true, false, false, true, false, false, true,
        false, true, false, false, true, false, true,
      ],
    },
  );

  glyphs.insert(
    '*',
    PixelFontGlyph {
      name: "*",
      width: 2,
      pixels: vec![
        true, true, true, true, false, false, false, false, false, false,
      ],
    },
  );

  glyphs.insert(
    '(',
    PixelFontGlyph {
      name: "(",
      width: 2,
      pixels: vec![
        false, true, true, false, true, false, true, false, false, true,
      ],
    },
  );

  glyphs.insert(
    ')',
    PixelFontGlyph {
      name: ")",
      width: 2,
      pixels: vec![
        true, false, false, true, false, true, false, true, true, false,
      ],
    },
  );

  glyphs.insert(
    '-',
    PixelFontGlyph {
      name: "-",
      width: 3,
      pixels: vec![
        false, false, false, false, false, false, true, true, true, false, false, false, false,
        false, false,
      ],
    },
  );

  glyphs.insert(
    '_',
    PixelFontGlyph {
      name: "_",
      width: 4,
      pixels: vec![
        false, false, false, false, false, false, false, false, false, false, false, false, false,
        false, false, false, true, true, true, true,
      ],
    },
  );

  glyphs.insert(
    '=',
    PixelFontGlyph {
      name: "=",
      width: 3,
      pixels: vec![
        false, false, false, true, true, true, false, false, false, true, true, true, false, false,
        false,
      ],
    },
  );

  glyphs.insert(
    '+',
    PixelFontGlyph {
      name: "+",
      width: 3,
      pixels: vec![
        false, false, false, false, true, false, true, true, true, false, true, false, false,
        false, false,
      ],
    },
  );

  glyphs.insert(
    '[',
    PixelFontGlyph {
      name: "[",
      width: 2,
      pixels: vec![
        true, true, true, false, true, false, true, false, true, true,
      ],
    },
  );

  glyphs.insert(
    ']',
    PixelFontGlyph {
      name: "]",
      width: 2,
      pixels: vec![
        true, true, false, true, false, true, false, true, true, true,
      ],
    },
  );

  glyphs.insert(
    '{',
    PixelFontGlyph {
      name: "{",
      width: 3,
      pixels: vec![
        false, true, true, false, true, false, true, false, false, false, true, false, false, true,
        true,
      ],
    },
  );

  glyphs.insert(
    '}',
    PixelFontGlyph {
      name: "}",
      width: 3,
      pixels: vec![
        true, true, false, false, true, false, false, false, true, false, true, false, true, true,
        false,
      ],
    },
  );

  glyphs.insert(
    '/',
    PixelFontGlyph {
      name: "/",
      width: 3,
      pixels: vec![
        false, false, true, false, false, true, false, true, false, true, false, false, true,
        false, false,
      ],
    },
  );

  glyphs.insert(
    '\\',
    PixelFontGlyph {
      name: "\\",
      width: 3,
      pixels: vec![
        true, false, false, true, false, false, false, true, false, false, false, true, false,
        false, true,
      ],
    },
  );

  glyphs.insert(
    '|',
    PixelFontGlyph {
      name: "|",
      width: 1,
      pixels: vec![true, true, true, true, true],
    },
  );

  glyphs.insert(
    '\'',
    PixelFontGlyph {
      name: "'",
      width: 1,
      pixels: vec![true, true, false, false, false],
    },
  );

  glyphs.insert(
    '"',
    PixelFontGlyph {
      name: "\"",
      width: 3,
      pixels: vec![
        true, false, true, true, false, true, false, false, false, false, false, false, false,
        false, false,
      ],
    },
  );

  glyphs.insert(
    '>',
    PixelFontGlyph {
      name: ">",
      width: 3,
      pixels: vec![
        true, false, false, false, true, false, false, false, true, false, true, false, true,
        false, false,
      ],
    },
  );

  glyphs.insert(
    '<',
    PixelFontGlyph {
      name: "<",
      width: 3,
      pixels: vec![
        false, false, true, false, true, false, true, false, false, false, true, false, false,
        false, true,
      ],
    },
  );

  glyphs.insert(
    '.',
    PixelFontGlyph {
      name: ".",
      width: 1,
      pixels: vec![false, false, false, false, true],
    },
  );

  glyphs.insert(
    ',',
    PixelFontGlyph {
      name: ",",
      width: 2,
      pixels: vec![
        false, false, false, false, false, false, false, true, true, false,
      ],
    },
  );

  glyphs.insert(
    '?',
    PixelFontGlyph {
      name: "?",
      width: 3,
      pixels: vec![
        true, true, false, false, false, true, false, true, false, false, false, false, false,
        true, false,
      ],
    },
  );

  glyphs.insert(
    ':',
    PixelFontGlyph {
      name: ":",
      width: 1,
      pixels: vec![false, true, false, true, false],
    },
  );

  glyphs.insert(
    ';',
    PixelFontGlyph {
      name: ";",
      width: 2,
      pixels: vec![
        false, false, false, true, false, false, false, true, true, false,
      ],
    },
  );

  glyphs.insert(
    '~',
    PixelFontGlyph {
      name: "~",
      width: 6,
      pixels: vec![
        false, false, false, false, false, false, false, true, true, false, false, true, true,
        false, false, true, true, false, false, false, false, false, false, false, false, false,
        false, false, false, false,
      ],
    },
  );

  glyphs.insert(
    '`',
    PixelFontGlyph {
      name: "`",
      width: 2,
      pixels: vec![
        true, false, false, true, false, false, false, false, false, false,
      ],
    },
  );

  glyphs.insert(
    '…',
    PixelFontGlyph {
      name: "…",
      width: 5,
      pixels: vec![
        false, false, false, false, false, false, false, false, false, false, false, false, false,
        false, false, false, false, false, false, false, true, false, true, false, true,
      ],
    },
  );

  PixelFontCharacterMap {
    name: "Sigi 5px Condensed Regular",
    height: 5,
    glyphs,
  }
});
